...
function StatusBadge({ status }: { status: Property["status"] }) {
  return (
    <span className={`inline-block border border-neutral-200 px-3 py-1 text-xs font-black uppercase tracking-wider ${STATUS_STYLES[status]}`}>
      {status}
    </span>
  )
}
...
function SpecTile({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex flex-col gap-1.5 border border-neutral-200 p-3">
      ...
      >
        ...
        <div className="relative shrink-0 border-b border-neutral-200">
          ...
          <div className="relative z-10 flex items-start justify-between gap-4 p-5 sm:p-6">
            <div className="flex items-center gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center border border-neutral-200 bg-black">
            ...
            <button
              type="button"
              onClick={onClose}
              className="shrink-0 border border-neutral-200 p-2 transition-colors hover:bg-black hover:text-white"
              aria-label="Close"
            >
        ...
        <div className="overflow-y-auto p-5 sm:p-6">
          <div className="flex flex-col gap-6">
            ...
            {/* Listing value hero */}
            <div className="border border-neutral-200 bg-black p-5 shadow-[0px_2px_8px_rgba(0,0,0,0.08)]">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {onFindBuyers && (
              <button
                ...
                className="flex flex-1 items-center justify-center gap-2 border border-neutral-200 bg-primary py-3.5 text-sm font-black uppercase tracking-wider text-black shadow-[0px_2px_8px_rgba(0,0,0,0.08)] transition-all hover:shadow-[0px_1px_3px_rgba(0,0,0,0.06)]"
              >
