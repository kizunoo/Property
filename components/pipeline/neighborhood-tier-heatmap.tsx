...
export function NeighborhoodTierHeatmap({ clients, onCellClick }: NeighborhoodTierHeatmapProps) {
  ...
  // The raw `clients` prop contains one row per evaluation (client × property),
  // so we keep only the first occurrence of each clientDbId (highest-E(x) / best-match row).
  ...
  if (neighborhoods.length === 0) {
    return (
      <div className="border-4 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        <HeatmapHeader onCellClick={onCellClick} />
  ...
  for (const c of uniqueClients) {
    ...
    // Use the best-match expectedValue from this evaluation row as the client's E(x) contribution
    cellMap.set(key, { ...cur, count: cur.count + 1, totalEx: cur.totalEx + c.expectedValue })
  }

  // Max count across all cells — used for intensity scaling
              ...
              {TIERS.map((tier, colIndex) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => count > 0 && onCellClick?.(nbhd, tier)}
                    onMouseEnter={(e) => {
                      if (count > 0) {
...
function HeatmapHeader({ onCellClick }: { onCellClick?: unknown }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b-4 border-black bg-black px-5 py-3.5 sm:px-6">
      ...
      <span className="border-2 border-black bg-primary px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-black sm:text-xs">
        {onCellClick ? "Click to filter" : "Live"}
      </span>
    </div>
  )
}
